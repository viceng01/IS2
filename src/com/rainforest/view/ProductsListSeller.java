...
public class ProductsListSeller extends JDialog implements ActionListener {
	...
	private void initGUI() {
		...
		/*
		 * for(int i = 0; i < 30; ++i) { tablaMedio.add(new ProductButton()); }
		 */
	...
	private void addProducts() {
		...
		if (u != null) {
			...
			for (int i = 0; i < ja.length(); i++) {
				...
				/*
				 * JLabel name = new JLabel("name: " + p.getString("name")); JLabel desc = new
				 * JLabel(p.getString("description")); JLabel price = new JLabel("price: " +
				 * String.valueOf(p.getFloat("price"))); JLabel amount = new JLabel("amount: " +
				 * String.valueOf(jo.getInt("amount")));
				 * 
				 * panel.add(name,BorderLayout.NORTH); panel.setToolTipText(desc.getText());
				 * panel.add(price,BorderLayout.CENTER); panel.add(amount,BorderLayout.SOUTH);
				 */
	...
	private void quit() {
		option = JOptionPane.showOptionDialog(this, "Are you sure you want to sign out?", "Sign Out",
