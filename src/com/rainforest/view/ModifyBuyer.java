...
public class ModifyBuyer extends JDialog implements ActionListener {
	...
	public ModifyBuyer(MainWindow mainWindow, ControlPanel cp, JDialog anterior) {
		mw = mainWindow;
		this.cp = cp;
		this.ant = anterior;
	...
	private void initGUI(MainWindow mainWindow) {
		...
		formPanel.setLayout(new GridLayout(8, 8));
		formPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.gray),
		...
		/*
		 * hGroup.addGroup(formLayout.createParallelGroup().addComponent(dir).
		 * addComponent(dni)
		 * .addComponent(tel).addComponent(pass).addComponent(email).addComponent(user))
		 * ;
		 * hGroup.addGroup(formLayout.createParallelGroup().addComponent(dirTextField).
		 * addComponent(dniTextField)
		 * .addComponent(telTextField).addComponent(emailTextField).addComponent(
		 * userTextFiled).addComponent(dirTextField));
		 */
		mainPanel.add(formPanel);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == submit) {
			if (dirTextField.getText().equals("") || telTextField.getText().equals("")
					|| userTextField.getText().equals("") || emailTextField.getText().equals("")
					|| passTextField.getPassword().length == 0)
				showErrorDialog("Some fields empty");
			else {// Si no hay ningun campo vacio comprobamos en primer lugar si se ha modificado
					// algo
				JSONObject o = u.serialize();
				boolean salir = false;
				if (!o.get("username").equals(userTextField.getText())) {// Se modifico usuario
					if (!mw.tryModifyUser(userTextField.getText(), dniTextField.getText())) {
						...
						u.getUserInfo().setUsername(userTextField.getText());
				}
				...
				if (o.getInt("tel") != aux) {// Si el telefono se ha modificado
					if (!mw.tryModifyTel(aux, dniTextField.getText())) {
						...
						u.getUserInfo().setTelephone(aux);
				...
				if (!o.get("email").equals(emailTextField.getText())) {// Si se modifico el email
					if (!mw.tryModifyEmail(emailTextField.getText(), dniTextField.getText())) {
						...
						u.getUserInfo().setEmail(emailTextField.getText());
				}
				if (!o.get("direction").equals(dirTextField.getText())) {// Si se modifico el email
					if (!mw.tryModifyDir(dirTextField.getText(), dniTextField.getText())) {
						...
						u.getUserInfo().setAddress(dirTextField.getText());
				...
				if (!o.getString("password").equals(passTextField.getText())) {
					if (!mw.tryModifyPass(passTextField.getText(), dniTextField.getText())) {
						...
						u.getUserInfo().setPassword(passTextField.getText());
				}
				if (!salir) {
					setVisible(false);
					ant.setVisible(true);
